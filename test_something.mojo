...
def test_compression_levels():
    ...
    for i in range(len(compression_levels)):
        ...
        zip_file.writestr("test.txt", test_data, zipfile.ZIP_DEFLATED, compresslevel=level)
        zip_file.close()
    ...
    assert_true(file_sizes[2] <= file_sizes[0], "Level 9 should compress at least as well as level 1")
...
def test_compression_level_progressive_write():
    ...
    zip_entry1 = zip_file1.open_to_write("progressive.txt", "w", zipfile.ZIP_DEFLATED, compresslevel=1)
    for i in range(len(test_data_parts)):
        zip_entry1.write((test_data_parts[i] * 50).as_bytes())  # Make it repetitive
    zip_entry1.close()
    zip_file1.close()
    
    # Create with level 9 (best compression)
    zip_file9 = zipfile.ZipFile(file_path_level9, "w")
    zip_entry9 = zip_file9.open_to_write("progressive.txt", "w", zipfile.ZIP_DEFLATED, compresslevel=9)
    for i in range(len(test_data_parts)):
    ...
    expected_content = (test_data_parts[0] * 50) + (test_data_parts[1] * 50) + (test_data_parts[2] * 50)
...
def test_compression_level_constants():
    ...
    zip_speed.writestr("test.txt", test_data, zipfile.ZIP_DEFLATED, compresslevel=zipfile.Z_BEST_SPEED)
    zip_speed.close()
    ...
    zip_compression.writestr("test.txt", test_data, zipfile.ZIP_DEFLATED, compresslevel=zipfile.Z_BEST_COMPRESSION)
    ...
    zip_default.writestr("test.txt", test_data, zipfile.ZIP_DEFLATED, compresslevel=zipfile.Z_DEFAULT_COMPRESSION)
    zip_default.close()
    
    # Verify all files can be read correctly
    for file_path in [file_path_speed, file_path_compression, file_path_default]:
        zip_read = zipfile.ZipFile(file_path, "r")
        file_reader = zip_read.open("test.txt", "r")
        content = file_reader.read()
        assert_equal(String(bytes=content), test_data)
